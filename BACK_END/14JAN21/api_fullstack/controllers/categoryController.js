...
module.exports = {
    ...
    getAllbyParent: async (req, res) => {
        try {
            const queryCate = `select c2.id , c2.title, c1.title as parent 
            from category c1 right join category c2 on c1.id = c2.parent_id`
            const result = await asyncQuery(queryCate)
    ...
    getAllbyChildren: async (req, res) => {
        try {
            const queryCate = `select c1.id, c1.title,group_concat(c2.title separator ',') as children 
            from category c1 left join category c2 on c2.parent_id = c1.id 
            group by c1.title order by c1.id`
            const result = await asyncQuery(queryCate)
    ...
    getCateDetParent: async (req, res) => {
        try {
            const parentQuery = `SELECT c1.id, c1.title, c1.parent_id, c2.title AS parent FROM category c1
